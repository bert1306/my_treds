# Развёртывание my spaces на VPS Beget (Ubuntu 24.04)
...
Чат по пространству и перевод по ссылкам работают через Ollama. На VPS с 2 ГБ RAM полная модель `llama3.2` (~2.3 ГБ) не поместится — появится ошибка «model requires more system memory». Используйте лёгкую модель:
...
В `.env` укажите: `OLLAMA_MODEL="llama3.2:1b"`. Затем перезапустите приложение: `pm2 restart my_treds`.

С 4+ ГБ RAM можно использовать `llama3.2` (в `.env`: `OLLAMA_MODEL="llama3.2"`).

Если Ollama не ставить — приложение будет работать, но чат и перевод по ссылкам выдадут сообщение, что Ollama недоступна.

**После перезагрузки сервера** Ollama не запускается сама — заново выполните на сервере: `ollama serve &` и при необходимости `ollama run <модель из .env>` (например `ollama run llama3.2:1b`). Чтобы Ollama поднималась автоматически, настройте systemd-сервис (см. документацию Ollama).
