...
- **Чат с помощником (LLM)** в контексте пространства: вопросы по смыслу, краткие изложения, пересказ по загруженному контенту. Используется локальная Ollama (по умолчанию `llama3.2`). Запросы дольше 30 с уходят в фон (до 2 одновременно, лимит задаётся `CHAT_BACKGROUND_JOBS_MAX`); чат остаётся доступным.
...
4. Перейти на страницу пространств `/threads`:
   ...
   - в блоке **«Чат с помощником»** задать вопрос по контенту (например: «сделай краткое изложение») — ответ даёт LLM по контексту пространства.
