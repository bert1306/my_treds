...
export async function POST(req: NextRequest) {
  ...
  const contextBlocks = items.map((i) => {
    const head = `[Пространство: ${i.thread.title}${i.title ? ` | ${i.title}` : ""}]\n`;
  ...
  const langRule =
    user.language === "ru"
      ? "КРИТИЧНО: весь ответ только по-русски. Латинскими только: названия организаций, аббревиатуры (AML, KYC, CFT), термины. Глаголы и связки — по-русски. Запрещено: Exist/need a/must have/Additionally/It's worth noting и любые английские фразы вместо русских. Без иероглифов."
  ...
  const sourceInstruction = hasContext
    ? `КРИТИЧНО: В приложении «пространство» — это папка с названием. Реально найденные пространства (список полный, других нет): ${threadNames}.

В ответе перечисли ТОЛЬКО эти названия пространств. ЗАПРЕЩЕНО перечислять как пространства: названия законов, документов, «Guidance for…», «Cabinet Decision…», «Procedures for…», «Central Bank & Organization…», «AML/CFT…» и т.п. — это фрагменты текста внутри пространства, а не пространства. Если найдено одно пространство — напиши один пункт. Формат: «Найдено пространство: [название]. [Одним предложением — о чём.] Перейти в пространство можно по ссылке в блоке «Найденные пространства» ниже.» Не приводи URL и не нумеруй длинные списки документов.`
    : "Результатов поиска по пространствам нет. Напиши коротко и чётко на русском, только кириллицей: «По запросу ничего не найдено в ваших пространствах. Попробуйте другие слова или добавьте контент в пространства.» Без иероглифов и слов на других языках.";
  ...
  const systemContent = `Ты помощник в приложении «my spaces». У пользователя есть сохранённые пространства с контентом. Ниже — фрагменты, найденные по запросу.
${tzLine}
${langRule} ${styleHint}

${hasContext ? "Контекст из пространств пользователя:" : "Результаты поиска:"}
${context || "(по запросу ничего не найдено в сохранённых пространствах)"}
